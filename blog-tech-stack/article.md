...
In the code above:
  ...
  1. `preprocess` calls my custom preprocessing function for the resulting Hiccup data structure to preform desired manipulations (described below)  
...
As I described in my [previous article](/post/blog-platform), I had set myself a requirement for keeping URL of the blog ([www.brotherus.net](https://www.brotherus.net/)) and URLs of individual articles (such as [https://www.brotherus.net/post/entry-to-chemistry-programming](https://www.brotherus.net/post/entry-to-chemistry-programming)) identical from transitioning from Wix to my custom blog-software.

My earlier static JavaScript application deployments I had used simply AWS S3 buckets that I configured for public web-hosting. For example for my [Ruletti-reframe game](post/ruletti-reborn) I created "roulette-reframe" bucket which after web-hosting enabling makes the game visible in URL [http://roulette-reframe.s3-website.eu-north-1.amazonaws.com/](http://roulette-reframe.s3-website.eu-north-1.amazonaws.com/). This works fine, but these URLs are ugly, difficult to remember, don't support custom domains and only work with HTTP, not HTTPS which modern browsers almost require. 

![AWS Logo](aws.webp)

Mapping S3 deployment to custom domain and HTTPS in AWS is not possible in the S3 service alone, but requires creation of cloud resources in several other services too:

  1. [Amazon CloudFront](https://aws.amazon.com/cloudfront/) content delivery network (CDN) service must be configured with the S3 bucket as content source. While the usage of custom domain in this case makes CloudFront necessary, it does provide the additional benefit of caching and slightly lower transfer latencies than S3 alone. 
...
Because AWS has dozens of services, each of these have numerous resource types and each resource numerous attributes, it used to be rather tedious to find out the right set of services, resources and resource-attributes needed to accomplish a given goal. When the best available help was Google, Stack Overflow and technical articles, one could easily end up on several incorrect paths in setting up the cloud resources. But with the aid of modern LLMs, this has become child's play: from a very high-level description of your goal ChatGPT, Gemini, Claude, Llama and DeepSeek provide exhaustive step-by-step guides for reaching your goal by navigating through the thousands of pages and settings of the AWS Portal.

For a project with multiple environments (like DEV, QA and PROD), bigger team, longer development life-cycle or more complex cloud resources, it would have made more sense to use some Infrastructure-As-Code (IAC) solution like [AWS CloudFormation](https://aws.amazon.com/cloudformation/). This would involve writing the list of required resources and their attributes with some IAC Coding language (a flavor of YAML in case of CloudFormation), so that executing this code creates the desired resources.
